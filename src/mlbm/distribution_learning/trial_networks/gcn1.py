...
class GraphCollisionLayer1(gnn.MessagePassing):
    def __init__(self, emb_dim, edge_dim, coord_diff=None,aggr="add", bias=True, residual_connection=False, activation=nn.Tanh):
        ...
        if self.coord_diff is not None:
            self.register_buffer("coord_diff_const", self.coord_diff)

    def forward(self, h, edge_index, x, omega):
        return (
            self.propagate(edge_index, h=h, x=x, omega=omega)
        )

    def message(self, h_i, h_j, x_i, x_j, omega_i):
        if self.coord_diff is None:
            coord_diff = torch.sum((x_i - x_j) ** 2, dim=1).unsqueeze(1)
        else:
            coord_diff = self.coord_diff_const.expand(h_i.shape[0], -1, -1)
        tmp = torch.cat([h_i, h_j, omega_i, coord_diff], dim=-1)
...
class GraphCollisionNetwork1(pl.LightningModule):
    def __init__(self, hparams, input_dim, edge_dim, output_dim, edge_index=None):
        super().__init__()
    ...
    def forward(self, h, edge_index, x, omega):
        ...
        h = self.encoder(h)
        h = self.layer1(h, edge_index, x, omega)
        h = self.layer1(h, edge_index, x, omega)
        h = self.decoder(h)
    ...
    def training_step(self, batch, batch_idx):
        ...
        h, edge_index, x, omega= batch.x, batch.edge_index, batch.pos, batch.omega
        h = torch.cat([batch.x, batch.weights], dim=1)
        out = self.forward(h, edge_index, x, omega) 
