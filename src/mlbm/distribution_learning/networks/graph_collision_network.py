...
from mlbm.distribution_learning.utils.loss_utilities import LossOption, get_loss_function

class GraphCollisionLayer(gnn.MessagePassing):
    def __init__(self, emb_dim, edge_dim, aggr='add', bias=True, residual_connection=False):
...
class GraphCollisionNetwork(pl.LightningModule):
    def __init__(self, hparams, input_dim, edge_dim, output_dim):
        ...
        self.encoder = nn.Sequential(nn.Linear(self.input_dim, self.emb_dim, bias=self.bias), nn.ReLU())
        self.layer1 = GraphCollisionLayer(emb_dim=self.emb_dim, edge_dim=self.edge_dim, aggr=self.aggr, bias=self.bias, residual_connection=self.residual_connection)
        self.layer2 = GraphCollisionLayer(emb_dim=self.emb_dim, edge_dim=self.edge_dim, aggr=self.aggr, bias=self.bias, residual_connection=self.residual_connection)
        self.decoder = nn.Sequential(nn.Linear(self.emb_dim, self.output_dim, bias=self.bias), nn.ReLU())
    ...
    def forward(self, h, edge_index, x, omega, edge_attr=None):
        ...
        fcorr = fpred - 1/9 * torch.sum(fpred - fpre, dim=1).unsqueeze(-1) - 1/6 * torch.einsum("dQ,dN->NQ", self.lattice_velocities_const, torch.einsum("dQ,NQ->dN", self.lattice_velocities_const, fpred - fpre))
