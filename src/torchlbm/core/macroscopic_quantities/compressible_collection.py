...
class CompressibleMacroscopicQuantityCalculationModule(nn.Module):
    ...
    def __init__(self, lattice_velocities: List[List[float]], cv, shifted_velx, shifted_vely) -> None:
        """The initializer.
        ...
        """
        super(CompressibleMacroscopicQuantityCalculationModule, self).__init__()
    ...
    def forward(self, node_data: CompressibleNodeData) -> CompressibleNodeData:
        ...
        """
        # print("Before ", node_data.moments.energy.shape)
        node_data.moments.density = torch.sum(node_data.distributions.vel_old_population, dim=0)
        node_data.moments.velocity = torch.einsum(
            "dQ,QNML->dNML", self.lattice_velocities_const, node_data.distributions.vel_old_population
        ) / node_data.moments.density.unsqueeze(0)
        node_data.moments.energy = torch.sum(node_data.distributions.temp_old_population, dim=0) / (2 * node_data.moments.density)
        # print(node_data.distributions.temp_old_population.shape)
        # print(torch.sum(node_data.distributions.temp_old_population, dim=0).shape)
        node_data.moments.temperature = (node_data.moments.energy - 0.5 * torch.sum(node_data.moments.velocity ** 2, dim=0)) / self.cv
        # print("After ", node_data.moments.energy.shape)
        return node_data
