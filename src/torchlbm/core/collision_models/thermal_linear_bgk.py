...
class ThermalEquilibriumCalculationModule(nn.Module):
    ...
    def __init__(self, lattice_velocities: List[List[float]], lattice_weights: List[float]) -> None:
        ...
        """
        super(ThermalEquilibriumCalculationModule, self).__init__()
    ...
    def forward(self, node_data: ThermalNodeData) -> ThermalNodeData:
        ...
        node_data.distributions.vel_new_population = node_data.moments.density.unsqueeze(0) * common_term
        node_data.distributions.temp_new_population = node_data.moments.temperature.unsqueeze(0) * common_term
