...
class ZeroGradientBoundaryUpdate(nn.Module):
    ...
    def forward(self, node_data: NodeData) -> NodeData:
        ...
        node_data.distributions.old_population = self.update_east_zero_gradient(node_data.distributions.old_population)
        # if self.is_west_zero_gradient:
        node_data.distributions.old_population = self.update_west_zero_gradient(node_data.distributions.old_population)

        # if self.is_north_zero_gradient:
        if self.dimension != 1:
            node_data.distributions.old_population = self.update_north_zero_gradient(node_data.distributions.old_population)
        # if self.is_south_zero_gradient:
        if self.dimension != 1:
            node_data.distributions.old_population = self.update_south_zero_gradient(node_data.distributions.old_population)

        # if self.is_top_zero_gradient:
        if self.dimension == 3:
            node_data.distributions.old_population = self.update_top_zero_gradient(node_data.distributions.old_population)
        # if self.is_bottom_zero_gradient:
        if self.dimension == 3:
            node_data.distributions.old_population = self.update_bottom_zero_gradient(node_data.distributions.old_population)
