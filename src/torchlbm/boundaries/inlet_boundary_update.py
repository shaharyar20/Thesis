...
class InletBoundaryUpdate(nn.Module):
    ...
    def __init__(
        self,
        is_east_inlet: bool,
        east_inlet_velocity: torch.Tensor,
        is_west_inlet: bool,
        west_inlet_velocity: torch.Tensor,
        is_north_inlet: bool,
        north_inlet_velocity: torch.Tensor,
        is_south_inlet: bool,
        south_inlet_velocity: torch.Tensor,
        is_top_inlet: bool,
        top_inlet_velocity: torch.Tensor,
        is_bottom_inlet: bool,
        bottom_inlet_velocity: torch.Tensor,
        num_halo_cells: int,
        ...
        Args:
            is_east_inlet (bool): Inidicator whether the east part of the computational domain is a wall.
            east_inlet_velocity (torch.Tensor): The wall velocity of the east part of the computational domain.
            is_west_inlet (bool): Inidicator whether the west part of the computational domain is a wall.
            west_inlet_velocity (torch.Tensor): The wall velocity of the west part of the computational domain.
            is_north_inlet (bool): Inidicator whether the north part of the computational domain is a wall.
            north_inlet_velocity (torch.Tensor): The wall velocity of the north part of the computational domain.
            is_south_inlet (bool): Inidicator whether the south part of the computational domain is a wall.
            south_inlet_velocity (torch.Tensor): The wall velocity of the south part of the computational domain.
            is_top_inlet (bool): Inidicator whether the top part of the computational domain is a wall.
            top_inlet_velocity (torch.Tensor): The wall velocity of the top part of the computational domain.
            is_bottom_inlet (bool): Inidicator whether the bottom part of the computational domain is a wall.
            bottom_inlet_velocity (torch.Tensor): The wall velocity of the bottom part of the computational domain.
        ...
        """
        super(InletBoundaryUpdate, self).__init__()

        self.is_east_inlet = is_east_inlet
        self._east_inlet_velocity = east_inlet_velocity
        self.register_buffer("east_inlet_velocity_const", self._east_inlet_velocity)
        self.is_west_inlet = is_west_inlet
        self._west_inlet_velocity = west_inlet_velocity
        self.register_buffer("west_inlet_velocity_const", self._west_inlet_velocity)
        self.is_north_inlet = is_north_inlet
        self._north_inlet_velocity = north_inlet_velocity
        self.register_buffer("north_inlet_velocity_const", self._north_inlet_velocity)
        self.is_south_inlet = is_south_inlet
        self._south_inlet_velocity = south_inlet_velocity
        self.register_buffer("south_inlet_velocity_const", self._south_inlet_velocity)
        self.is_top_inlet = is_top_inlet
        self._top_inlet_velocity = top_inlet_velocity
        self.register_buffer("top_inlet_velocity_const", self._top_inlet_velocity)
        self.is_bottom_inlet = is_bottom_inlet
        self._bottom_inlet_velocity = bottom_inlet_velocity
        self.register_buffer("bottom_inlet_velocity_const", self._bottom_inlet_velocity)
    ...
    # fmt: off
    def update_east_inlet(self, population: torch.Tensor, density: torch.Tensor) -> torch.Tensor:
        ...
        for east_index in self.east_velocities:
            ...
            wall_velocity_projection = torch.dot(self.east_inlet_velocity_const, self.lattice_velocity_const[:, east_index])
    ...
    def update_west_inlet(self, population: torch.Tensor, density: torch.Tensor) -> torch.Tensor:
        ...
        for west_index in self.west_velocities:
            ...
            wall_velocity_projection = torch.dot(self.west_inlet_velocity_const, self.lattice_velocity_const[:, west_index])
            inlet_velocity_dot = torch.dot(self.west_inlet_velocity_const, self.west_inlet_velocity_const)
            ...
            population[
                ...
                self.access_indices[2][1] if self.dimension == 3 else 0:
                self.access_indices[2][2] if self.dimension == 3 else 1,
            ] = - f_slice + 2.0 * weight * density_slice * ( 1.0 + 0.5 * 9.0 * wall_velocity_projection**2 - 0.5 * 3.0 * inlet_velocity_dot**2)
        return population

    def update_north_inlet(self, population: torch.Tensor, density: torch.Tensor) -> torch.Tensor:
        ...
        for north_index in self.north_velocities:
            ...
            wall_velocity_projection = torch.dot(self.north_inlet_velocity_const, self.lattice_velocity_const[:, north_index])
    ...
    def update_south_inlet(self, population: torch.Tensor, density: torch.Tensor) -> torch.Tensor:
        ...
        for south_index in self.south_velocities:
            ...
            wall_velocity_projection = torch.dot(self.south_inlet_velocity_const, self.lattice_velocity_const[:, south_index])
    ...
    def update_top_inlet(self, population: torch.Tensor, density: torch.Tensor) -> torch.Tensor:
        ...
        for top_index in self.top_velocities:
            ...
            wall_velocity_projection = torch.dot(self.top_inlet_velocity_const, self.lattice_velocity_const[:, top_index])
    ...
    def update_bottom_inlet(self, population: torch.Tensor, density: torch.Tensor) -> torch.Tensor:
        ...
        for bottom_index in self.bottom_velocities:
            ...
            wall_velocity_projection = torch.dot(self.bottom_inlet_velocity_const, self.lattice_velocity_const[:, bottom_index])
