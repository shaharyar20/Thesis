...
class ThermalPeriodicBoundaryUpdate(nn.Module):
    ...
    def __init__(
        ...
        """
        super(ThermalPeriodicBoundaryUpdate, self).__init__()
            ...
            for j in y_range:
                for k in z_range:
                    if (i != 1 and self.is_i_periodic) or (j != 1 and self.is_j_periodic) or (k != 1 and self.is_k_periodic):
                        vel_distribution_halo_cells = self.get_halo_array_multi_field(discrete_velocities, i, j, k)
                        # print(distribution_halo_cells.shape)
                        discrete_velocities = self.exchange_halo_in_array_multi_field(
                            discrete_velocities,
                            vel_distribution_halo_cells,
                            -1 * (i - 1) + 1,
                            -1 * (j - 1) + 1,
                            -1 * (k - 1) + 1,
                        )
                        temp_distribution_halo_cells = self.get_halo_array_multi_field(discrete_temperatures, i, j, k)
                        discrete_temperatures = self.exchange_halo_in_array_multi_field(
                            discrete_temperatures,
                            temp_distribution_halo_cells,
                            -1 * (i - 1) + 1,
                            -1 * (j - 1) + 1,
                            -1 * (k - 1) + 1,
                        )
