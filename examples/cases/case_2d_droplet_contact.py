...
class ShearLayerInitialCondition(TorchlbmInitialCondition):
    ...
    def get_initial_velocity(self, X, Y, Z):
        ...
        a = (ul + ug)/2
        b = (ul - ug)/2
        V_physical = a - b*torch.tanh(2*(torch.sqrt((X - x1)**2 + (Y - y1)**2) - r0)/w)
...
def main():
    ...
    simulation_setup["Domain"]["CellsPerNode"].value = 256
    ...
    simulation_setup["Physics"]["EndTime"].value = 4.0
    ...
    simulation_setup["Multiphase"]["SolidDensity"].value = 0.1
