...
class PorousMediaFlowInitialCondition(TorchlbmInitialCondition):
    ...
    def get_bounce_back_mask(self, X, Y, Z):
        ...
        aneurysm_mask = np.load("/home/jwinter/Test_Meshes/Realistic_Aneu/3D/Resolution_400/grid_400.npy").astype(int)
        aneurysm_mask = torch.from_numpy(aneurysm_mask)
        # aneurysm_mask = torch.transpose(aneurysm_mask, 1, 0)
        # print(mask.shape)
        mask[1:1+aneurysm_mask.shape[0],3:3+aneurysm_mask.shape[1],3:3+aneurysm_mask.shape[2]] = 1-aneurysm_mask
...
def main():
    ...
    simulation_setup["Domain"]["NodeRatio"].value = [20, 7, 13]
    ...
    # simulation_setup["Domain"]["BoundaryConditions"]["West"]["OutletDensity"].value = inflow_velocity
    ...
    simulation_setup["Output"]["OutputTimeInterval"].value = 0.05 #0.2
    ...
    simulation_setup["Physics"]["EndTime"].value = 8.0
