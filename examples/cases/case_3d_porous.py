...
class PorousMediaFlowInitialCondition(TorchlbmInitialCondition):
    def __init__(self, torchlbm_setup: TorchlbmSetup, seed: int) -> None:
    ...
    def get_bounce_back_mask(self, X, Y, Z):
        ...
        n_circles = 30
        torch.random.seed = self.seed
        for i in range(n_circles):
            x0 = torch.rand([1]).item()
            x0 = x0 * 0.72 + 0.14
            y0 = torch.rand([1]).item()
            y0 = y0 * 0.72 + 0.14
            z0 = torch.rand([1]).item()
            z0 = z0 * 0.72 + 0.14
            r = 0.06 + 0.06 * torch.rand([1]).item()
            mask = torch.where(torch.sqrt((X-x0)*(X-x0)+(Y-y0)*(Y-y0)+(Z-z0)*(Z-z0)) < r, 1, mask)
...
def main():
    ...
    end_time = 5
    resolution_list = [32, 64]
    resolution_list = [32]

    for resolution in resolution_list:
        for i in range(400,800):

            simulation_setup = TorchlbmSetup(f"PorousMedia_{resolution}_{i}")
            simulation_setup["Domain"]["Dimension"].value = "3D"
            simulation_setup["Domain"]["NodeSize"].value = 1.0
            simulation_setup["Domain"]["CellsPerNode"].value = resolution
            ...
            simulation_setup["Output"]["OutputTimeInterval"].value = end_time
            ...
            simulation_setup["Output"]["Velocity"]["Types"].value = ["PyTorch"]
            simulation_setup["Output"]["Density"]["Active"].value = True
            simulation_setup["Output"]["Density"]["Types"].value = ["PyTorch"]
            simulation_setup["Output"]["BounceBackMask"]["Active"].value = True
            simulation_setup["Output"]["BounceBackMask"]["Types"].value = ["PyTorch"]

            simulation_setup["Physics"]["MachNumber"].value = 0.15
            simulation_setup["Physics"]["EndTime"].value = end_time
            simulation_setup["Physics"]["CharacteristicVelocityPu"].value = 4.0
            simulation_setup["Physics"]["KinematicViscosityPu"].value = 0.005
            ...
            initial_condition = PorousMediaFlowInitialCondition(simulation_setup, seed = i * 11 + resolution*7)
