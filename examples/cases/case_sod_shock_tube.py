...
def main():
    ...
    simulation_setup = TorchlbmSetup("SodShockTube")
    simulation_setup["Domain"]["Dimension"].value = "2D"
    simulation_setup["Domain"]["NodeSize"].value = 5.0  #2 * torch.pi
    simulation_setup["Domain"]["CellsPerNode"].value = 5
    simulation_setup["Domain"]["NumHaloCells"].value = 1
    simulation_setup["Domain"]["NodeRatio"].value = [600, 1, 1]
    simulation_setup["Domain"]["BoundaryConditions"]["East"]["Type"].value = "ZeroGradient" #"Wall" #"ZeroGradient"
    simulation_setup["Domain"]["BoundaryConditions"]["East"]["WallVelocity"].value = [0.0, 0.0, 0.0]
    simulation_setup["Domain"]["BoundaryConditions"]["West"]["Type"].value = "ZeroGradient" #"Wall" #"ZeroGradient"
    ...
    simulation_setup["Thermal"]["ThermalConductivity"].value = 0.025*2.0/0.71
    simulation_setup["Thermal"]["Cp"].value = 2.0
    simulation_setup["Thermal"]["Cv"].value = 1.0
    simulation_setup["Thermal"]["Viscosity"].value = 0.025
    simulation_setup["Thermal"]["ShiftedVelocityX"].value = 0.1
    simulation_setup["Thermal"]["ShiftedVelocityY"].value = 0.1
    simulation_setup["Thermal"]["BoundaryConditions"]["East"]["WallTemperature"].value = 0.025
    simulation_setup["Thermal"]["BoundaryConditions"]["West"]["WallTemperature"].value = 0.2
    ...
    simulation_setup["Physics"]["EndTime"].value = 300.0
